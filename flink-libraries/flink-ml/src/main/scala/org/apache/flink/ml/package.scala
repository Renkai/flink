...
package object ml {
  ...
  implicit class RichDataSet[T](dataSet: DataSet[T]) {
    def mapWithBcVariable[B, O: TypeInformation: ClassTag](
        ...
        fun: (T, B) => O)
      : DataSet[O] = {
      dataSet.map(new BroadcastSingleElementMapper[T, B, O](dataSet.scalaCheckAndclean(fun)))
    ...
    def filterWithBcVariable[B, O](broadcastVariable: DataSet[B])(fun: (T, B) => Boolean)
      : DataSet[T] = {
      dataSet.filter(new BroadcastSingleElementFilter[T, B](dataSet.scalaCheckAndclean(fun)))
        .withBroadcastSet(broadcastVariable, "broadcastVariable")
    }
    ...
    def mapWithBcVariableIteration[B, O: TypeInformation: ClassTag](
      ...
      : DataSet[O] = {
      dataSet.map(new BroadcastSingleElementMapperWithIteration[T, B, O](dataSet.scalaCheckAndclean(fun)))
        .withBroadcastSet(broadcastVariable, "broadcastVariable")
    }
  }
